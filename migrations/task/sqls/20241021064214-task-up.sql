...
-- 1. 新增：新增六筆用戶資料，資料如下：
--     1. 用戶名稱為`李燕容`，Email 為`lee2000@hexschooltest.io`,`USER`
--     2. 用戶名稱為`王小明`，Email 為`wXlTq@hexschooltest.io`,`USER`
...
select
	...
	sum(distinct("CREDIT_PURCHASE".purchased_credits)) - count(distinct("COURSE_BOOKING")) as "remaining_credit"
from
	"CREDIT_PURCHASE"
inner join
	"COURSE_BOOKING" on "CREDIT_PURCHASE".user_id = "COURSE_BOOKING".user_id
where
...
select
	"USER".name as 教練名稱,
	"COACH".experience_years as 經驗年資,
	"SKILL".name as 技能
